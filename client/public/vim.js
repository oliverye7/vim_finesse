  ...
  init: function () {
    ...
    emscriptenRuntimeInitialized
      .then(function () {
        ...
        guiWasmHandleDrop = Module.cwrap("gui_wasm_handle_drop", null, [
          "string",
        ]);
        guiWasmSetClipAvail = Module.cwrap("gui_wasm_set_clip_avail", null, [
          "boolean",
        ]);
        guiWasmDoCmdline = Module.cwrap("gui_wasm_do_cmdline", "boolean", [
          "string",
        ]);
      ...
      SharedBuffers.prototype.takeBuffer = function (status, bufId) {
        ...
        if (buf === undefined) {
          throw new Error(
            "Received " +
              statusName(status) +
              " event but no shared buffer for buffer ID " +
              bufId
          );
        }
    ...
    var VimWasmRuntime = (function () {
                ...
                switch (e.name) {
                  case "ExitStatus":
                    ...
                    _this
                      ...
                      .catch(function (err) {
                        console.error(
                          "worker: Could not shutdown filesystem:",
                          err
                        );
                      })
                      .then(function () {
      ...
      VimWasmRuntime.prototype.start = function (msg) {
        ...
        this.perf = msg.perf;
        var willPrepare = this.prepareFileSystem(
          msg.persistent,
          msg.dirs,
          msg.files,
          msg.fetchFiles
      ...
      VimWasmRuntime.prototype.exportFile = function (fullpath) {
        try {
          ...
          this.sendMessage(
            { kind: "export", path: fullpath, contents: contents },
            [contents]
          );
          return true;
        } catch (err) {
      ...
      VimWasmRuntime.prototype.readClipboard = function () {
        ...
        var buffer = this.sharedBufs.takeBuffer(
          STATUS_NOTIFY_CLIPBOARD_WRITE_COMPLETE,
          bufId
        );
      ...
      VimWasmRuntime.prototype.evalJavaScriptFunc = function (
        func,
        argsJson,
        notifyOnly
      ) {
        debug(
          "Will send function and args to main for jsevalfunc():",
          func,
          argsJson,
          notifyOnly
        );
      ...
      VimWasmRuntime.prototype.prepareFileSystem = function (
        ...
        for (
          var _b = 0, persistentDirs_1 = persistentDirs;
          _b < persistentDirs_1.length;
          _b++
        ) {
      ...
      VimWasmRuntime.prototype.waitForStatusChanged = function (timeout) {
        ...
        if (
          Atomics.wait(this.buffer, 0, STATUS_NOT_SET, timeout) === "timed-out"
        ) {
      ...
      VimWasmRuntime.prototype.handleEvent = function (s) {
        switch (s) {
          ...
          default:
            throw new Error(
              "Cannot handle event " + statusName(s) + " (" + s + ")"
            );
        }
      };
      ...
      VimWasmRuntime.prototype.handleOpenFileWriteComplete = function () {
        ...
        debug(
          "Read open file write complete event payload with",
          idx * 4,
          "bytes"
        );
        var buffer = this.sharedBufs.takeBuffer(
          STATUS_NOTIFY_OPEN_FILE_BUF_COMPLETE,
          bufId
        );
        debug(
          "Handle file",
...
function _vimwasm_wait_for_event(timeout) {
  return VW.runtime.waitAndHandleEventFromMain(
...
var _gui_wasm_handle_keydown = (Module["_gui_wasm_handle_keydown"] =
  function () {
...
var _gui_wasm_set_clip_avail = (Module["_gui_wasm_set_clip_avail"] =
  function () {

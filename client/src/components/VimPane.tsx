...
function Commandline({ vim }: { vim: VimWasm | null }) {
...
function DownloadBuffer({ vim }: { vim: VimWasm | null }) {
    const onClick = useCallback(() => {
        if (vim !== null) {
            vim.cmdline('export');
        }
...
function Control({ vim }: { vim: VimWasm | null }) {
    return (
        <div className="box">
...
const VIM_WASM_AVAILABLITY_MESSAGE = true;
